...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    std::transform(word1.begin(), word1.end(), tp1.begin(), toupper);
    std::transform(word2.begin(), word2.end(), tp2.begin(), toupper);
    
    if (dict.find(tp1) != dict.end() && dict.find(tp2) != dict.end()
        && dict.find(tp1)->second == dict.find(tp2)->second){
        return true;
    }
    else return false;
